...
This document describes how to enable and use the Plan Mode feature with TodoList middleware in AgentFlow.
...
AgentFlow uses custom `system_prompt` and `tool_description` for the TodoListMiddleware that match the overall AgentFlow prompt style:

### System Prompt Features
- Uses XML tags (`<todo_list_system>`) for structure consistency with AgentFlow's main prompt
...
- TodoList middleware uses LangChain's built-in `TodoListMiddleware` with **custom AgentFlow-style prompts**
...
- Custom prompts emphasize the same principles as AgentFlow's main system prompt (clarity, action-oriented, critical rules)
