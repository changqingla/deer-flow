...
def client(mock_app_config):
    """Create a AgentFlowClient with mocked config loading."""
...
class TestExtractText:
    def test_string(self):
        assert AgentFlowClient._extract_text("hello") == "hello"
    ...
    def test_list_text_blocks(self):
        ...
        assert AgentFlowClient._extract_text(content) == "first\nsecond"

    def test_list_plain_strings(self):
        assert AgentFlowClient._extract_text(["a", "b"]) == "a\nb"

    def test_empty_list(self):
        assert AgentFlowClient._extract_text([]) == ""

    def test_other_type(self):
        assert AgentFlowClient._extract_text(42) == "42"
...
class TestUploads:
    def test_upload_files(self, client):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with patch.object(AgentFlowClient, "_get_uploads_dir", return_value=uploads_dir):
    ...
    def test_delete_upload_not_found(self, client):
        with tempfile.TemporaryDirectory() as tmp:
            with patch.object(AgentFlowClient, "_get_uploads_dir", return_value=Path(tmp)):
                with pytest.raises(FileNotFoundError):
...
class TestScenarioThreadIsolation:
    ...
    def test_uploads_isolated_per_thread(self, client):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with patch.object(AgentFlowClient, "_get_uploads_dir", side_effect=get_dir):
...
class TestScenarioEdgeCases:
    ...
    def test_upload_convertible_file_conversion_failure(self, client):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with (
                patch.object(AgentFlowClient, "_get_uploads_dir", return_value=uploads_dir),
