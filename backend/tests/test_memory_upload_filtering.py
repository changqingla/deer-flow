...
class TestStripUploadMentionsFromMemory:
    ...
    def test_uploading_a_test_file_removed(self):
        ...
        mem = self._make_memory("User conducted a hands-on test by uploading a test file titled 'test_AgentFlow_memory_bug.txt'. User is also learning Python.")
        result = _strip_upload_mentions_from_memory(mem)
        summary = result["user"]["topOfMind"]["summary"]
        assert "test_AgentFlow_memory_bug.txt" not in summary
