...
|----------------|-------------|----------------------------------|----------------------------------|
| Gateway API    | 8001        | `$AGENT_FLOW_GATEWAY_URL`          | REST endpoints (models, skills, memory, uploads) |
| LangGraph API  | 2024        | `$AGENT_FLOW_LANGGRAPH_URL`        | Agent threads, runs, streaming   |
...
| `AGENT_FLOW_GATEWAY_URL`  | `${AGENT_FLOW_URL}`                        | Gateway API base (models, skills, memory, uploads) |
| `AGENT_FLOW_LANGGRAPH_URL`| `${AGENT_FLOW_URL}/api/langgraph`          | LangGraph API base (threads, runs) |

When making curl calls, always resolve the URL like this:

```bash
# Resolve base URLs from env (do this FIRST before any API call)
AGENT_FLOW_URL="${AGENT_FLOW_URL:-http://localhost:2026}"
AGENT_FLOW_GATEWAY_URL="${AGENT_FLOW_GATEWAY_URL:-$AGENT_FLOW_URL}"
AGENT_FLOW_LANGGRAPH_URL="${AGENT_FLOW_LANGGRAPH_URL:-$AGENT_FLOW_URL/api/langgraph}"
```
...
```bash
curl -s "$AGENT_FLOW_GATEWAY_URL/health"
```
...
```bash
curl -s -X POST "$AGENT_FLOW_LANGGRAPH_URL/threads" \
...
```bash
curl -s -N -X POST "$AGENT_FLOW_LANGGRAPH_URL/threads/<thread_id>/runs/stream" \
  -H "Content-Type: application/json" \
  -d '{
...
```bash
curl -s "$AGENT_FLOW_GATEWAY_URL/api/models"
```
...
```bash
curl -s "$AGENT_FLOW_GATEWAY_URL/api/skills"
```
...
```bash
curl -s -X PUT "$AGENT_FLOW_GATEWAY_URL/api/skills/<skill_name>" \
  -H "Content-Type: application/json" \
  -d '{"enabled": true}'
```
...
```bash
curl -s "$AGENT_FLOW_GATEWAY_URL/api/agents"
```
...
```bash
curl -s "$AGENT_FLOW_GATEWAY_URL/api/memory"
```
...
```bash
curl -s -X POST "$AGENT_FLOW_GATEWAY_URL/api/threads/<thread_id>/uploads" \
...
```bash
curl -s "$AGENT_FLOW_GATEWAY_URL/api/threads/<thread_id>/uploads/list"
```

### 11. Get Thread History

```bash
curl -s "$AGENT_FLOW_LANGGRAPH_URL/threads/<thread_id>/history"
```

### 12. List Threads

```bash
curl -s -X POST "$AGENT_FLOW_LANGGRAPH_URL/threads/search" \
  -H "Content-Type: application/json" \
