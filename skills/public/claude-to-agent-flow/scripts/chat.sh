#!/usr/bin/env bash
# chat.sh — Send a message to AgentFlow and collect the streaming response.
#
# Usage:
...
#   AGENT_FLOW_URL=http://host:2026 bash chat.sh "hi"   # custom endpoint
#
# Environment variables:
#   AGENT_FLOW_URL          — Unified proxy base URL (default: http://localhost:2026)
#   AGENT_FLOW_GATEWAY_URL  — Gateway API base URL (default: $AGENT_FLOW_URL)
#   AGENT_FLOW_LANGGRAPH_URL — LangGraph API base URL (default: $AGENT_FLOW_URL/api/langgraph)
#
...
GATEWAY_URL="${AGENT_FLOW_GATEWAY_URL:-$AGENT_FLOW_URL}"
LANGGRAPH_URL="${AGENT_FLOW_LANGGRAPH_URL:-$AGENT_FLOW_URL/api/langgraph}"
...
if [ "$HTTP_CODE" = "000" ] || [ "$HTTP_CODE" -ge 400 ]; then
  echo "ERROR: AgentFlow is not reachable at ${GATEWAY_URL} (HTTP ${HTTP_CODE})" >&2
  echo "Make sure AgentFlow is running. Start it with: cd <AgentFlow-dir> && make dev" >&2
  exit 1
fi
...
if result_messages is not None:
    ...
    for event_type, data_str in events:
        if event_type == "error":
            print(f"ERROR from AgentFlow: {data_str}", file=sys.stderr)
            sys.exit(1)
